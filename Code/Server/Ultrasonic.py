...
class Ultrasonic:
    def __init__(self):
        ...
        self.trig_GPIO = GPIO(self.trigger_pin, "out")
        self.echo_GPIO = GPIO(self.echo_pin, "in")

    def pulseIn(self, gpio, level, timeOut):  # obtain pulse time of a pin under timeOut
        t0 = time.time()
    ...
    def get_distance(self):  # get the measurement results of ultrasonic module,with unit: cm
        ...
        for i in range(5):
            self.trig_GPIO.write(True)  # make trigger_pin output 10us HIGH level
            time.sleep(0.00001)  # 10us
            self.trig_GPIO.write(False)  # make trigger_pin output LOW level
            pingTime = self.pulseIn(self.echo_GPIO, True, self.timeOut)  # read plus time of echo_pin
...
if __name__ == '__main__':
    ...
    try:
        ...
        ultrasonic.pwm_S.setServoPwm('1', 90)
